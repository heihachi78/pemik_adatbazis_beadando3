...
docker exec -u postgres pgp psql -p 5432 -U cms -d cms -t -c "CREATE PUBLICATION pub_fin_data FOR TABLE ONLY public.fin_publicated_data (account_number, bank_account_id, first_name, last_name, birth_name, person_id, partner_case_number, amount, case_id, created_at) WITH (publish = 'insert, update, delete, truncate', publish_via_partition_root = false);"
